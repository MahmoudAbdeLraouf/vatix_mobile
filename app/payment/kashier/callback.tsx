...
import {
  confirmKashierRedirect,
  finalizePaymentSuccess,
  pollKashierStatus,
  type PaymentContext,
} from '@/lib/payment'

export default function KashierCallbackScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (Object.keys(redirectParams).length > 0) {
        await confirmKashierRedirect(paymentId, redirectParams)
      }

      const res = await pollKashierStatus(paymentId)
