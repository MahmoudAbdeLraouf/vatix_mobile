...
export function SubscriptionExpiringDialog({ visible, onClose, subscriptionEndsAt }: Props) {
  const { t } = useLocale()
  ...
  const formattedDate = useMemo(() => {
    ...
    if (Number.isNaN(d.getTime())) return subscriptionEndsAt
    try {
  ...
  }, [subscriptionEndsAt, ar])

  const body = t.subscriptionExpiring.body.replace('{date}', formattedDate)

  function handleDismiss() {
    if (busy) return
    setBusy(true)
    markActionDismissed('subscription_expiring_dialog')
    onClose()
  }
  ...
  function handleGo() {
    if (busy) return
    setBusy(true)
    markActionSeen('subscription_expiring_dialog')
    onClose()
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text style={[styles.description, dirStyle]}>{body}</Text>

              <View style={[styles.actionsRow, rowDir]}>
                <View style={styles.actionCell}>
                  <Button
                    label={t.subscriptionExpiring.laterCta}
                    variant="outline"
                    size="lg"
                ...
                <View style={styles.actionCell}>
                  <Button
                    label={t.subscriptionExpiring.topUpCta}
                    variant="y"
                    size="lg"
                    onPress={handleGo}
                    disabled={busy}
